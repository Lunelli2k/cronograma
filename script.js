// --- CONFIGURAÇÃO E ESTADO GLOBAL ---
    ...
    function getWeekDates() {
        const week = [], today = new Date(), dayOfWeek = (today.getDay() === 0) ? 6 : today.getDay() - 1, monday = new Date(today);
    ...
    function generateMermaidCode(board) {
        // --- NOVO TEMA "CARTOON" ---
        const mermaidTheme = `%%{init: { 
            "theme": "base", 
            "themeVariables": { 
                "fontFamily": "Inter, sans-serif", 
                "fontSize": "15px",
                "primaryColor": "${document.body.dataset.theme === 'dark' ? '#27272a' : '#f4f4f5'}", 
                "primaryTextColor": "${document.body.dataset.theme === 'dark' ? '#e4e4e7' : '#18181b'}", 
                "lineColor": "${document.body.dataset.theme === 'dark' ? '#a1a1aa' : '#71717a'}",
                "nodeBorder": "var(--accent)"
            } 
        } }%%\n`;

        // Definição de estilo para tarefas concluídas
        let mermaidStr = mermaidTheme + `flowchart TD\n  classDef done fill:#dcfce7,stroke:#16a34a,color:#15803d,fontWeight:600\n`;
    ...
    function renderChecklist() {
        const board = getActiveBoard(), container = document.getElementById('flow-checklist-container'), today = new Date(), todayIndex = today.getDay(), todayName = today.toLocaleDateString('pt-BR', { weekday: 'long' });
