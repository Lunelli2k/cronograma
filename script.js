...
async function duplicateBoard(id) {
    ...
    for (let i = 0; i < 7; i++)(nB.days[i] || []).forEach(t => {
        t.id = uid('t');
        t.done = false;
    });
...
function generateMermaidCode(board) {
    const mermaidTheme = `%%{init:{"theme":"base","themeVariables":{"fontFamily":"'Courier New', monospace","fontSize":"14px","primaryColor":"#0d1117","primaryTextColor":"#39ff14","lineColor":"#39ff14","nodeBorder":"#39ff14"}}}%%\n`;
    let mermaidStr = mermaidTheme + `flowchart TD\n  classDef done fill:#0a2d0b,stroke:#39ff14,color:#81ff7e\n`;
